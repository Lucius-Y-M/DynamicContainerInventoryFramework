...
namespace {
	static void AddLeveledListToContainer(RE::TESLeveledList* list, RE::TESObjectREFR* a_container, uint32_t a_count) {
		...
		/* Here is the original code (as of 2.2.1):
			...
			and it adds EVERY permissible item from the queried leveled list to the container.

		
			Since a leveled list is meant to produce only one item from a list of items,
			perhaps adding just one item is much better?


			Also, we may want to consider "chanceNone" (how likely nothing will be added)
			(as of 2.2.1, even if I try to add a LeveledList that has 90 chanceNone, the item(s) still get added 100% of the times)
		*/


			auto chanceNone = list->chanceNone;
			if (chanceNone >= 100) return;			// no one should ever set a leveledList's chanceNone to 100, but if yes, our work is done here

			short shouldCalcThisList = clib_util::RNG().Generate<short>(0, 100);
			if (shouldCalcThisList < chanceNone) return;
		...
		if (result.size() < 1) return;
				...
				/*
					...
					but there's a chance the ThingToAdd is a nullptr

					so instead, can shuffle the whole thing (maybe more compute costly?)
					then go from begin to end, get the first that is valid
				*/				


				/*
					for shuffling, I tried:

						std::shuffle(result.begin(), result.end(), some_seed);

					but kept getting error C2794:

						'type': is not a member of any direct or indirect base class of 'std::_Invoke_traits_zero<void,_Urng &>'

					So ended up basically writing std::shuffle again in a different name
					(probably something eluding me)
				*/
				constexpr auto ShuffleResult = [&](RE::BSScrapArray<RE::CALCED_OBJECT> arr, auto seed) {
					...
					for (int i = (endIdx - startIdx) - 1; i > 0; --i) {
...
namespace ContainerManager {
	bool ContainerManager::IsRuleValid(SwapRule* a_rule, RE::TESObjectREFR* a_ref) {
		...
		if (a_rule->chance == 0) return false;
