		...
		for (auto& data : rules) {
			...
			if (conditions) {
				...
				if (questConditionField) {
					...
					if (auto& stageDoneField = questConditionField["stageDone"]) {
						for (auto& element : stageDoneField) {
							uint16_t stageNum = (uint16_t) element.asUInt64();
			...
			for (auto& change : changes) {
				...
				auto& chanceField = conditions["chance"];				
				...
				if (chanceField && chanceField.isUInt()) {
					newRule.chance = (uint8_t) std::min((int) chanceField.asUInt(), 100);
				}
				else {
					newRule.chance = 100;	// default behavior: if chance declared, but not valid, ignore value & set to 100?
				}
